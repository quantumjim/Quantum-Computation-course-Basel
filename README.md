# Quantum Computation and Error Correction

**Course Overview:** This course package consists of two related offerings: a set of [lectures on quantum theory](https://vorlesungsverzeichnis.unibas.ch/de/vorlesungsverzeichnis?id=302620) (4 credits) and a corresponding technical [exercise component](https://vorlesungsverzeichnis.unibas.ch/de/vorlesungsverzeichnis?id=302621) (2 credits). Both are hosted at the University of Basel.

### 📚 Lecture Series Details

* **Credits/Level:** 4 Credits
* **Instructors:** Dr. James Wootton, Dr. Pierre Fromholz, and Dr. Daniel Bultrini of [Moth Quantum](https://mothquantum.com/).

**Key Dates & Topics:**

| Date | Instructor | Topic |
| :--- | :--- | :--- |
| September 15 | TBD | Introduction to Quantum Computation |
| September 22 | Dr. Fromholz | The Qubit and Quantum Mechanics Fundamentals |
| September 29 | Dr. Bultrini | Mathematics for Quantum I (Linear Algebra Review) |
| October 6 | Dr. Wootton | Mathematics for Quantum II (Advanced Topics) |
| October 13 | Dr. Bultrini | Multiqubits, Universality, and Quantum Circuits I |
| October 20 | Dr. Fromholz | Multiqubits, Universality, and Quantum Circuits II |
| October 27 | Dr. Bultrini | Algorithms I: Fourier Transform (FT) |
| November 3 | Dr. Fromholz | Algorithms II: Phase Estimation (PE) |
| November 10 | Dr. Fromholz | Algorithms III: Period Finding and Order-Finding |
| November 17 | Dr. Bultrini | Noise, Decoherence, and Error Mitigation |
| November 24 | Dr. Wootton | Quantum Error Correction I (Foundations) |
| December 1 | Dr. Wootton | Quantum Error Correction II (Codes and Theory) |
| December 8 | Dr. Wootton | Quantum Error Correction III (Advanced Codes/Protocols) |

**Course Materials:**
* All lectures will be recorded and made available shortly after the session in the dedicated `Lectures` folder.
* Supplementary lecture notes (slides, manuscripts, or typed summaries) will also be provided in this same folder.

### 🛠️ Exercises Component Details

* **Credits/Level:** 2 Credits
* **Supervision:** Supervised by the main lecture group.
* **Teaching Assistants (TAs):** Viktoriia Pinchenkova, Sarthak Sarthak, Xinru Tang, Zoé McIntyre, Hari Priya Pettugani, Zoltan György, Shahan Hawatian.

**Key Dates & Topics:**

| Date | Topic | Instructor
| :--- | :--- | :--- |
| September 15 | Basics (No Submission Required) | Haripriya Pettugani |
| September 22 | Introduction to Quantum Circuits | Haripriya Pettugani |
| September 29 | The Qubit and Quantum Mechanics Fundamentals | Haripriya Pettugani |
| October 6 | Mathematics for Quantum I | Viktoriia Pinchenkova |
| October 13 | Mathematics for Quantum II | Viktoriia Pinchenkova |
| October 20 | Multiqubits, Universality, and Circuits I | Sarthak Girdhar |
| October 27 | Multiqubits, Universality, and Circuits II | Sarthak Girdhar |
| November 3 | Algorithms I: Fourier Transform (FT) | Dr. Zoe McIntyre |
| November 10 | Algorithms II: Phase Estimation (PE) | Dr. Zoe McIntyre |
| November 17 | Algorithms III: Period Finding/Order-Finding | Shahan Hawatian |
| November 24 | Noise and Error Mitigation | Shahan Hawatian |
| December 1 | Quantum Error Correction I |  Xinru Tang |
| December 8 | Quantum Error Correction II |  Xinru Tang |

**Submission Details:**
* Exercise sheets are provided in advance (Please see the `Exercises` folder).
* **Preparation:** Students must prepare the assigned material *before* the corresponding correction session, with the exception of the first session.
* **Corrections:** Each exercise correction will be made available shortly after its associated correction session.

---

### 🎓 Grading and Examination Structure

#### **1. Lecture Exam (Written)**
* **Date & Time:** December 15th (at the same time as the scheduled lectures).
* **Scope:** The exam covers all material presented in the Exercise sessions, excluding the final topic (QEC III).
* **Grading:** Graded out of 6 points. A passing grade is 4 or more.

#### **2. Exercises Evaluation (Effort-Based)**
There is no traditional "exam" for the exercises; instead, evaluation focuses on sustained effort and participation.

**Submission Process:** Students must submit their completed exercise sheets via ADAM (or alternative method approved by a TA) by the night prior to the correction session.

**Passing Criteria ("Good Effort"):**
* The TAs will evaluate if the student demonstrated a "good effort" for at least 80% of the questions on the sheet.
* **Crucially:** Passing requires demonstrating *effort*, not necessarily correct answers. It is possible to pass without answering any question correctly, and vice versa.
* There is no requirement for attendance, but it is strongly advised that students attend the lectures and excercise sessions. 

**Final Grade:**
* To successfully complete the exercises component, a student must achieve a passing grade for 70% or more of the sheets (i.e., passing at least 9 out of 12 sheets).

**Note:** While it is administratively possible to take only one of two courses, and pass it, it is difficult and not recommended as knowledge of both courses is necessary to succed in both evaluation mechanism.

---

### ❓ Student Support and Questions

* **Lecture Material/Exam:** For questions regarding lecture content, please address them first to the corresponding lecturer. If unable to reach the assigned instructor, please contact Prof. Pierre Fromholz (`pierre.fromholz AT unibas.ch`).
* **Exercises/Grading:** For all questions concerning assignments and grading processes, please address them first to the designated TA for that week. If unable to reach the specific TA, please contact Zoé McIntyre (`zoe.mcintyre AT unibas.ch`).

---
***
